...
enum ExpresieData {
    Operatie(Box<ExpresieData>, char, Box<ExpresieData>),
        ...
        match ch {
            ...
            _ => {
                ...
                match ch {
                    ...
                    '(' => {
                        if !numere.is_empty() {
                            return Err("Operatorul ( trebuie sa aiba inainte un operator.");
                        }
    ...
    while caractere.len() > *poz {
        match &caractere[*poz] {
            CaracterePosibile::Rad => {
                *poz += 1;
                let nr = if let Some(ok) = numere.pop() {
                    ok
                } else {
                    return Err("Nu se poate efectua aceasta operatie.");
                };
                numere.push(ExpresieData::Rad(Box::new(nr)));
            }
            CaracterePosibile::Tangenta => {
                *poz += 1;
                ...
                numere.push(ExpresieData::Tangenta(Box::new(nr)));
            }
            CaracterePosibile::Log => {
                *poz += 1;
                ...
                numere.push(ExpresieData::Log(Box::new(nr)));
            }
            CaracterePosibile::Cos => {
                *poz += 1;
                ...
                numere.push(ExpresieData::Cos(Box::new(nr)));
            }
            CaracterePosibile::Sin => {
                *poz += 1;
                ...
                numere.push(ExpresieData::Sin(Box::new(nr)));
            }
...
fn calculare(operatie: &ExpresieData) -> Result<f64, &'static str> {
    match operatie {
        ExpresieData::Operatie(partea_st, varf_op, partea_dr) => {
            ...
            println!("={}{}{}", st_rezultat, varf_op, dr_rezultat);
            println!("={}", rezultat_final);
            Ok(rezultat_final)
        }
        ExpresieData::Numar(nr) => Ok(*nr),
        ExpresieData::Rad(nr) => {
            if let Some(nr) = nr.as_ref() {
                Ok(nr.sqrt())
            } else {
                Err("Unexpected None value in Rad operation")
            }
        }
        ExpresieData::Log(nr) => {
            if let Some(nr) = nr.as_ref() {
                ...
                Err("Unexpected None value in Log operation")
            }
        }
        ExpresieData::Sin(nr) => {
            if let Some(nr) = nr.as_ref() {
    ...
    match io::stdin().read_line(&mut operatie_de_calculat) {
        ...
        Ok(_) => {
            ...
            let operatie_data: String = operatie_de_calculat
                ...
                .collect();
            println!("Operatie fara whitespaces: {}", operatie_data);
            ...
            match formare_operatia_finala(&operatie_data) {
                ...
                Ok(caractere) => {
                    ...
                    match impartire_operatii(&mut caractere.clone(), &mut poz) {
                        ...
                        Ok(operatie) => {
                            match calculare(&operatie) {
                                Err(eroare) => {
                                    eprintln!("A existat o eroare la calcularea operatiei data: {eroare}");
                                }
                                Ok(_) => {
